...
- **TTS playback teardown must stop EVERY player it created, not just the latest, or spam-tapping stacks overlapping voices.** A playToken supersedes which request "wins", but silencing the loser is separate: on native, `createAudioPlayer().play()` can still fire if `remove()` lands while the clip is loading, so a superseded clip starts after teardown. Track all native players in a Set and `pause()`+`remove()` every one on teardown; also skip `play()` entirely when `token !== playToken`. On web, re-check the token after `await el.play()` and pause+cleanup if superseded.
  - **Why:** user reported stacked voices on rapid taps even though network-level fetch cancellation already worked — the bug was purely client-side playback overlap.
- **Never touch the `expo-audio` recorder's native object in an unmount cleanup without try/catch.** Reading the `audioRecorder.isRecording` getter (or calling any method) during unmount throws `NativeSharedObjectNotFoundException` synchronously once expo-audio has released the native shared object — and a `.catch()` on a later `.stop()` promise does NOT catch that synchronous getter throw, so it surfaces as an uncaught redbox on the device that breaks the mic.
  ...
  - **How to apply:** track recording state in a ref (mirror every `setIsRecording`), read the ref (not the native getter) in cleanup, wrap native calls in try/catch, and give the cleanup effect empty deps so it only runs on true unmount (the recorder from `useAudioRecorder` is stable). This boilerplate is duplicated verbatim in EVERY screen using `useAudioRecorder` (conversation + vocab-study so far) — when adding a new voice screen, copy the hardened cleanup, and the same back-mid-screen crash will recur if you reintroduce the native `isRecording` getter read.
